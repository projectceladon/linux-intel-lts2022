				...
				const struct pci_device_id *pci_id)
{
	...
	section_addr = GUEST_SHM_PADDR + 0x1000;
	section_addr = (section_addr + (min_align - 1)) & ~(min_align - 1);
	section_sz = vi_data->fact->size * 0x1000 - ((section_addr - GUEST_SHM_PADDR - 0x1000 + (min_align - 1))  & ~(min_align - 1));
