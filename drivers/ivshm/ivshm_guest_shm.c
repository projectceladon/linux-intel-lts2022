...
static int probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	idev->info.dev_mmio = GUEST_SHM_PADDR + 0x1000;
	idev->info.dev_mmio = (idev->info.dev_mmio + (min_align - 1)) & ~(min_align - 1);
	idev->info.dev_mmio_len = idev->fact->size * 0x1000 - ((idev->info.dev_mmio - GUEST_SHM_PADDR - 0x1000 + (min_align - 1)) & ~(min_align - 1));
