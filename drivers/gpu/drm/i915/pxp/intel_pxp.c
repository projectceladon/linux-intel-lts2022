...
static int __pxp_global_teardown_final(struct intel_pxp *pxp)
{
	...
	drm_info(&pxp->ctrl_gt->i915->drm, "PXP: teardown for suspend/fini");
	/*
...
static int __pxp_global_teardown_restart(struct intel_pxp *pxp)
{
	...
	drm_info(&pxp->ctrl_gt->i915->drm, "PXP: teardown for restart");
	/*
			   ...
			   struct prelim_drm_i915_pxp_tee_io_message_params *params)
{
	...
	char *in = (char *) msg_in;
	char *out = (char *)msg_out;
	trace_printk("input buffer size:%d\n", params->msg_in_size);
	for(int i = 0; i < params->msg_in_size;i++) {
		trace_printk("%d", in[i]);
	}
	trace_printk("\n");
	trace_printk("output buffer size:%d\n", params->msg_out_buf_size);
	for(int i = 0; i < params->msg_out_buf_size;i++) {
		trace_printk("%d", out[i]);
	}
	trace_printk("\n");
					...
					struct drm_file *drmfile)
{
	...
	drm_info(&pxp->ctrl_gt->i915->drm, "Handled host request (%d)\n", ret);
